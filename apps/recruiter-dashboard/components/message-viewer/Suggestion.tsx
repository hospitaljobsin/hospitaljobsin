...
export function Suggestion({
	...
	return (
		...
		>
			{partial ? <Spinner color="default" size="sm" /> : <span>{title}</span>}
		</Button>
	);
}
		...
		try {
			const numOfSuggestionsInstructions =
				config.minSuggestions === 0
					? `Produce up to ${config.maxSuggestions} suggestions. If there are no highly relevant suggestions you can think of, provide an empty array.`
			...
			const result = await extract({
				...
				stream: ({ status, args }) => {
					...
					for (let i = 0; i < suggestions.length; i++) {
						...
						const partial =
							i === suggestions.length - 1 && status !== "complete";
